...
def average_split_meta(
    ...
    for n in range(1, nRandom+1): 
        ...
        for sn in screen_names: # SPLIT INTO POS NEG #
            ...
            # df_LFC_LFC3D[f"{pref}_neg"] = df_LFC_LFC3D[f"{pref}"].apply(lambda x: float(x) if x != '-' and float(x) < 0 else np.nan)
            # df_LFC_LFC3D[f"{pref}_pos"] = df_LFC_LFC3D[f"{pref}"].apply(lambda x: float(x) if x != '-' and float(x) > 0 else np.nan)
            new_col_neg_list.append(df_LFC_LFC3D[f"{pref}"].apply(lambda x: float(x) if x != '-' and float(x) < 0 else np.nan).rename(f"{pref}_neg"))
            new_col_pos_list.append(df_LFC_LFC3D[f"{pref}"].apply(lambda x: float(x) if x != '-' and float(x) > 0 else np.nan).rename(f"{pref}_pos"))
        df_LFC_LFC3D = pd.concat([df_LFC_LFC3D] + new_col_neg_list + new_col_pos_list, axis=1)
...
def znorm_meta(
    ...
    result_data = {f'{aggr_func_name}_{score_type}_{sign}_{pthr_str}_{suffix}': [] 
                    for sign in ['neg', 'pos'] for suffix in ['z', 'p', 'psig'] for pthr_str in pthrs_str}

    for colname, param, sign in zip(colnames, params, ['neg', 'pos']): 
        signals_dict = df_bidir_meta[colname].to_dict()
        for pthr, pthr_str in zip(pthrs, pthrs_str): 
            for i in range(len(df_bidir_meta)):
                ...
                # Append results to the dictionary
                result_data[f'{aggr_func_name}_{score_type}_{sign}_{pthr_str}_z'].append(signal_z)
                result_data[f'{aggr_func_name}_{score_type}_{sign}_{pthr_str}_p'].append(signal_p)
                result_data[f'{aggr_func_name}_{score_type}_{sign}_{pthr_str}_psig'].append(signal_plabel)
