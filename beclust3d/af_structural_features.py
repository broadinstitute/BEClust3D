...
def af_structural_features(
        ...
        # fetch fasta #
        ffile = input_uniid + '.fasta'
        _ = wget.download(f'https://rest.uniprot.org/uniprotkb/{ffile}', out='')

        # fasta to list file #
        filedir = Path(workdir + input_gene)
        uFasta_file = filedir / ffile
        uFasta = open(uFasta_file, "r")
        header = uFasta.readline() # skip header

        uFasta_list_file = filedir / f"{input_gene}_{input_uniid}.tsv"
        uFasta_list = open(uFasta_list_file, "w")
        ...
        # fetch alphafold #
        affile = structureid + '.fasta'
        _ = wget.download(f'https://alphafold.ebi.ac.uk/files/{affile}', out='')
        af_filename = f"AF_{input_uniid}.pdb"
        os.rename(affile, af_filename)
        structureid = f"AF_{input_uniid}_v4"

        # Preprocess AF to keep Atoms and DSSP #

        af_source_filename = filedir / f"AF_{input_uniid}.pdb"
        af_source_file = open(af_source_filename, "r")
        ...
        af_processed_filename = filedir / f"{structureid}_processed.pdb"
        ...
        alphafold_filename = filedir / f"{structureid}_processed.pdb"
        alphafold_pdb = PandasPdb()
        alphafold_pdb.read_pdb(alphafold_filename)
        atom_df = alphafold_pdb.df['ATOM']
        # open fasta list file, combine fasta and pdb
        fastalist_filename = filedir / f"{input_gene}_{input_uniid}.tsv"
        fasta_df = pd.read_csv(fastalist_filename, sep = '\t')
        # out data file
        coord_pLDDT_filename = filedir / f"{structureid}_coord.tsv"
        coord_pLDDT_file = open(coord_pLDDT_filename, 'w')
        coord_pLDDT_file.write('\t'.join(['unipos', 'unires', 'x_coord', 'y_coord', 'z_coord', 'bfactor_pLDDT']+ '\n'))
        ...
        df_aamap = pd.DataFrame(data=aamap)
        # open alphafold dssp file and read
        alphafold_dssp_filename = filedir / f"{structureid}_processed.dssp"
        # parse DSSP file into a dictionary
        parser = parseDSSP(alphafold_dssp_filename)
        parser.parse()
        # dictionary to dataframe
        pddict = parser.dictTodataframe()
        # collect infor for the relevant chain 
        ...
        fastalist_filename = workdir + input_gene + "/" + input_gene + "_" + input_uniid + ".tsv";
        # fasta file 
        fasta_df = pd.read_csv(fastalist_filename, sep = '\t')
        
        # out data file
        dssp_output_filename = filedir / f"{structureid}_dssp_parsed.tsv"
        dssp_output_file = open(dssp_output_filename, 'w')
        dssp_output_file.write('\t'.join(['unipos', 'unires', 'SS9', 'SS3', 'ACC', 'RSA', 'exposure', 'PHI', 'normPHI', 'PSI', 'normPSI']) + '\n')
        output_data = []
        for i in range(0,len(fasta_df)):
                unipos = fasta_df.at[i, 'unipos']
                uniaa = fasta_df.at[i, 'unires']
                ...
                if len(pddict_ch_entry) == 0:
                        ...
                        if norm_ASA < 0.05:  
                                exposure = "core"
                        elif (norm_ASA >= 0.05) and (norm_ASA < 0.25): 
                                exposure = "buried"
                        elif (norm_ASA >= 0.25) and (norm_ASA < 0.5): 
                                exposure = "medburied"
                        elif (norm_ASA >= 0.5) and (norm_ASA < 0.75): 
                                exposure = "medexposed"
                        else: 
                                exposure = "exposed"
                ...
                output_data_entry = '\t'.join([str(unipos), str(uniaa), str(dssp_SS9), str(dssp_SS3), str(dssp_ASA), str(norm_ASA), 
        ...
        coord_filename = filedir / f"{structureid}_coord.tsv"
        df_coord = pd.read_csv(coord_filename, sep = "\t")
        dssp_filename = filedir / f"{structureid}_dssp_parsed.tsv"
        df_dssp = pd.read_csv(dssp_filename, sep = '\t')

        # Counts of amino acids at 8A distance #

        taa_wise_contact_count = []
        taa_wise_naa = []
        taa_wise_naa_positions = []
        for taa in range(0, len(df_coord)):
                ...
                for naa in range(0, len(df_coord)):
                        if taa != naa:
                                ...
                                dis_x_sqr = (n_xcoord - t_xcoord) * (n_xcoord - t_xcoord)
                                dis_y_sqr = (n_ycoord - t_ycoord) * (n_ycoord - t_ycoord)
                                dis_z_sqr = (n_zcoord - t_zcoord) * (n_zcoord - t_zcoord)
                                
                                pairwise_dist = math.sqrt(dis_x_sqr + dis_y_sqr + dis_z_sqr)
                                if pairwise_dist <= 6.0:
                ...
                taa_contact_count = len(taa_all_pairwise_dis)
                taa_wise_contact_count.append(taa_contact_count) 
                taa_naas = ';'.join(taa_all_naas)
                taa_wise_naa.append(taa_naas)
                taa_naas_positions = ';'.join(taa_all_naas_positions)
                taa_wise_naa_positions.append(taa_naas_positions)
        ...
        for aa in range(0, len(df_coord_dssp)):
                ...
                sum_taa_naa_dBurial = 0    
                norm_sum_taa_naa_dBurial = 0
        ...
        for i in range(0, len(df_coord_dssp)):
                ...
                if pLDDT < 50:
                        pLDDT_discrete = 'very low'
                elif (pLDDT < 70) and (pLDDT >= 50):
                        pLDDT_discrete = 'low'
                elif (pLDDT <= 90) and (pLDDT >= 70):
                        pLDDT_discrete = 'confident'
                elif pLDDT > 90:
                        pLDDT_discrete = 'high'
                arr_pLDDT_discrete.append(pLDDT_discrete)
        df_coord_dssp['pLDDT_dis'] = arr_pLDDT_discrete

        coord_dssp_filename = filedir / f"{structureid}_coord_struc_features.tsv"
        df_coord_dssp.to_csv(coord_dssp_filename, "\t", index=False)
