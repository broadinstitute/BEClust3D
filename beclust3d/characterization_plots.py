...
def RSA_vs_pLDDT_scatter(df_filtered, input_gene, 
                         plot_name, 
...
def hits_feature_barplot(df, category_col,
                         values_cols, values_vals, value_names, xname, input_gene, 
                         plot_name, plot_type='Count', colors = ['darkred', 'darkblue'], 
                         ):
    """
    Description
        Generate hit count barplot for specified feature (ex. RSA, pLDDT, Domain, etc.)
    ...
    plt.title(f"{input_gene} {xname} Hit Barplot")
